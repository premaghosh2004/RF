...
const FindRoomie: React.FC = () => {
  ...
  const [rentRange, setRentRange] = useState([15000]);
  const [city, setCity] = useState('Kolkata'); // Default to Kolkata
  const [gender, setGender] = useState<'Male' | 'Female' | 'Any' | undefined>('Any');
  const [food, setFood] = useState<'Vegetarian' | 'Non-Vegetarian' | 'Any' | undefined>('Any');
  const [duration, setDuration] = useState<'1-3 months' | '3-6 months' | '6-12 months' | '12+ months' | 'Flexible' | undefined>('Flexible');

  const { profiles, loading, error, setParams, refresh } = useProfiles({ 
    limit: 12, 
    city: 'Kolkata',
    sortBy: 'recent' 
  });
  ...
  useEffect(() => {
    setParams(prev => ({
      ...
      foodPreference: food === 'Any' ? undefined : food,
      duration: duration === 'Flexible' ? undefined : duration,
      minRent: 3000,
      maxRent: rentRange[0]
    }));
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4">
            Find Your <span className="text-gradient">Perfect Roommate in Kolkata</span>
          </h1>
          <p className="text-muted-foreground">
            Connect with compatible roommates in Kolkata and surrounding areas
          </p>
        </div>
...
/>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={food} onValueChange={(v) => setFood(v as any)}>
                    <SelectTrigger className="bg-background">
                      <SelectValue placeholder="Any" />
                    </SelectTrigger>
                    <SelectContent className="bg-popover">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <Card key={i} className="bg-card border-border h-80 animate-pulse">
                ...
                <div className="col-span-full text-center py-12">
                  <p className="text-muted-foreground">No roommates found in Kolkata. Try adjusting your filters.</p>
                </div>
              ) : (
                profiles.map((profile) => (
                  <Card key={profile._id} className="bg-card border-border hover-scale card-shadow overflow-hidden group">
                    <div className="relative">
                      <img 
                        src={profile.roomDetails?.images?.[0] || 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=800'} 
                        alt="Room" 
                      ...
                      {profile.compatibility && profile.compatibility >= 90 && (
                      ...
                      <div className="space-y-3 mb-4">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold flex items-center gap-1">
                            ...
                            ₹{(profile.roomDetails?.rent || profile.preferences?.rentRange?.max)?.toLocaleString()}/month
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
